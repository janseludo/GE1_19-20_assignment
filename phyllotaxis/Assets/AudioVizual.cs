...
public class AudioVizual : MonoBehaviour
{
    ...
    void CreateVisualiser()
    {
        ...
        /*
        //AudioanAlizer bands length divided by mathF pi goes in to theta
        float theta = (Mathf.PI * 2.0f) / (float)AudioAnalyzer.bands.Length;

        //initialize shere object
        GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
       
        sphere.transform.parent = this.transform;
        sphere.GetComponent<Renderer>().material.color = Color.HSVToRGB((float)AudioAnalyzer.bands.Length, 1, 1);
