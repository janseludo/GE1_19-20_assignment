...
{
    public float beat;              //what spectrum value to trigger a beat
    ...
    //Subclasses to inject thier own update without completely overwriting update()
    public virtual void OnUpdate()
    {
        ...
        if (m_previousAudioValue > beat && m_audioValue <= beat)
        {
            if (m_timer > timeStep)         // if minimum beat interval is reached
                OnBeat();
        }
